...
Eliminate patients without an initial period of at least 3 years of no seizures, no epilepsy, and no antiseizure medication prescription
```{r}
# First episode of seizure or epilepsy
first_episode_seizure_or_epilepsy <- IP_ED %>% 
  filter(epilepsy_encounter==1) %>% 
  arrange(ENROLID, SVCDATE) %>% 
  distinct(ENROLID, .keep_all=TRUE) %>% 
  select(ENROLID, SVCDATE) %>% 
  rename(date_first_seizure_or_epilepsy = SVCDATE) %>% 
  collect()




# First ASM prescription
first_ASM_prescription <- patients_with_epilepsy_and_ASM_no_gaps %>% 
  arrange(ENROLID, SVCDATE) %>% 
  distinct(ENROLID, .keep_all=TRUE) %>% 
  select(ENROLID, SVCDATE, AGE) %>% 
  rename(date_first_ASM_prescription = SVCDATE) %>% 
  collect()




#Identify the period of 3 years with no epilepsy, seizures, or ASM prescription

#################################2006#################################
enrollment_2006 <- ms_t_2006 %>% 
  select(ENROLID, DTSTART, DTEND, MEMDAYS) %>% 
  mutate(across(c(ENROLID, MEMDAYS), as.numeric)) %>%
  filter(ENROLID %in% !!patients_with_epilepsy_and_ASM_no_gaps$ENROLID) %>% 
  collect() %>% 
...
# Enrollment 
enrollment <- bind_rows(enrollment_2006, enrollment_2007, enrollment_2008, enrollment_2009, enrollment_2010, enrollment_2011, enrollment_2012, enrollment_2013, enrollment_2014, enrollment_2015, enrollment_2016, enrollment_2017, enrollment_2018, enrollment_2019, enrollment_2020, enrollment_2021, enrollment_2022) %>% 
  arrange(ENROLID, DTSTART) %>% 
  distinct(ENROLID, .keep_all=TRUE) %>% 
  left_join(first_episode_seizure_or_epilepsy, by="ENROLID") %>% 
  mutate(baseline_period = as.numeric(date_first_seizure_or_epilepsy - DTSTART)) %>% 
  left_join(first_ASM_prescription, by="ENROLID") %>% 
  mutate(baseline_period_before_first_ASM = as.numeric(date_first_ASM_prescription - DTSTART)) %>% 
  mutate(baseline_period_3_years = case_when(
    ( (baseline_period_before_first_ASM >= 0) & (AGE == 0) ) ~ "yes",
    ( (baseline_period_before_first_ASM >= 0) & (AGE == 1) ) ~ "yes",
    ( (baseline_period >= 365.24219) & (baseline_period_before_first_ASM >= 0) & (AGE == 2) ) ~ "yes",
    ( (baseline_period >= 2*365.24219) & (baseline_period_before_first_ASM >= 0) & (AGE == 3) ) ~ "yes",
    ( (baseline_period >= 3*365.24219) & (baseline_period_before_first_ASM >= 0) & (AGE > 3) ) ~ "yes",
    TRUE ~ "no")) %>% 
  collect()
...
# Reconvert the databases to reflect a baseline period of 3 years
enrollment_baseline_3_years <- enrollment %>% 
  filter(baseline_period_3_years == "yes") %>% 
  distinct(ENROLID) %>% 
  collect()
  
  
patients_with_epilepsy_and_ASM_no_gaps_baseline_3_years <- patients_with_epilepsy_and_ASM_no_gaps %>% 
  filter(ENROLID %in% !!enrollment_baseline_3_years$ENROLID) %>% 
  collect()


patients_with_epilepsy_and_ASM_no_gaps_baseline_3_years_n <- patients_with_epilepsy_and_ASM_no_gaps_baseline_3_years %>% distinct(ENROLID) %>% nrow()
patients_with_epilepsy_and_ASM_no_gaps_baseline_3_years_n


refractory_epilepsy_baseline_3_years <- refractory_epilepsy %>% 
  filter(ENROLID %in% !!enrollment_baseline_3_years$ENROLID) %>% 
  collect()


epilepsy_encounters_data_baseline_3_years <- epilepsy_encounters_data %>% 
  filter(ENROLID %in% !!enrollment_baseline_3_years$ENROLID) %>% 
  collect()  


Dementia_baseline_3_years <- Dementia %>% 
  filter(ENROLID %in% !!enrollment_baseline_3_years$ENROLID) %>% 
  collect()


chronic_respiratory_failure_baseline_3_years <- chronic_respiratory_failure %>% 
  filter(ENROLID %in% !!enrollment_baseline_3_years$ENROLID) %>% 
  collect()


epilepsy_baseline_3_years <- epilepsy %>% 
  filter(ENROLID %in% !!enrollment_baseline_3_years$ENROLID) %>% 
  collect()


all_epilepsy_encounters_data_baseline_3_years <- all_epilepsy_encounters_data %>% 
  filter(ENROLID %in% !!enrollment_baseline_3_years$ENROLID) %>% 
  collect()
```
...
rescue_medication_epilepsy_2006 <- ms_d_2006 %>% 
  filter(ENROLID %in% !!patients_with_epilepsy_and_ASM_no_gaps_baseline_3_years$ENROLID) %>% 
...
rescue_medication_patients_n / patients_with_epilepsy_and_ASM_no_gaps_baseline_3_years_n
```
...
demographics <- bind_rows(demographics_2006, demographics_2007, demographics_2008, demographics_2009, demographics_2010, demographics_2011, demographics_2012, demographics_2013, demographics_2014, demographics_2015, demographics_2016, demographics_2017, demographics_2018, demographics_2019, demographics_2020, demographics_2021, demographics_2022) %>% 
  ...
  filter(ENROLID>0) %>%    
  distinct(ENROLID, .keep_all=TRUE) %>% 
  left_join(patients_with_epilepsy_and_ASM_no_gaps_baseline_3_years, by=c("ENROLID", "DOBYR")) %>% 
  arrange(ENROLID, SVCDATE) %>% 
  group_by(ENROLID) %>% 
...
demographics <- demographics %>% 
  mutate(refractory_epilepsy_case = case_when(
    ENROLID %in% !!refractory_epilepsy_baseline_3_years$ENROLID ~ "refractory",
    TRUE ~ "non-refractory"
  )) %>% 
  collect()
...
demographics <- demographics %>%
  left_join(epilepsy_encounters_data_baseline_3_years, by="ENROLID") %>% 
...
demographics <- demographics %>% 
  left_join(Dementia_baseline_3_years, by="ENROLID") %>% 
  collect()
...
demographics <- demographics %>% 
  left_join(chronic_respiratory_failure_baseline_3_years, by="ENROLID") %>% 
  collect()
...
proportion_with_rescue_medication_per_year_plot_detail <- ggplot(data=proportion_with_rescue_medication_per_year,
  ...
  theme(legend.position="none", panel.background=element_blank(), panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.border=element_blank(), axis.ticks=element_blank(),
        ...
        axis.title=element_text(size=20, face="bold")) +
  scale_x_continuous(name="Year", breaks=2006:2022) +
  scale_y_continuous(name="Proportion", breaks=seq(0, 0.2, 0.025), limits = c(0, 0.2))
...
epilepsy_first_diagnosis <- epilepsy_baseline_3_years %>% 
  arrange(ENROLID, SVCDATE) %>% 
  distinct(ENROLID, .keep_all=TRUE) %>% 
...
rescue_prescriptions_after_epilepsy_IP_ED_encounter <- inflation_adjusted_rescue_medication_epilepsy %>% 
  ...
  left_join(all_epilepsy_encounters_data_baseline_3_years, by="ENROLID") %>% 
...
rescue_medication_patient_cost_per_year_by_generic_plot <- ggplot(data=rescue_medication_cost_per_year_by_generic) + 
  ...
  theme(legend.position="none", panel.background=element_blank(), panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.border=element_blank(), axis.ticks=element_blank(),
        ...
        axis.title=element_text(size=20, face="bold")) +
  scale_x_continuous(name="Year", breaks=2006:2022) +
  scale_y_continuous(name="Median (p25-p75) cost ($)", breaks=seq(0, 125, 25), limits = c(0, 125))
